...
void main() {
  group('ReceiptRepositoryImpl', () {
    test('uses API datasource for receipt list when API succeeds', () async {
      ...
      final repository = ReceiptRepositoryImpl(apiDataSource: apiDataSource);
    ...
    test('throws when receipt list API fails', () async {
      final apiDataSource = _FakeReceiptApiDataSource(
    ...
    test('throws when receipt detail API fails', () async {
      final apiDataSource = _FakeReceiptApiDataSource(
      ...
      await expectLater(
        () => repository.getReceiptById('api-rcp-004'),
        throwsA(isA<StateError>()),
      );
